...
namespace sam
{
    ...
    bool BaseSamModel::GenerateMask(const cv::Mat& image, const std::vector<std::pair<int, int>>& points,
    ...
    bool BaseSamModel::GenerateMask(const cv::Mat& image, const std::vector<BBox2D>& boxes, cv::Mat& result,
        bool isRGB)
    {
        // 0. check
    ...
    std::future<cv::Mat> BaseSamModel::GenerateMaskAsync(const cv::Mat& image, const std::vector<BBox2D>& boxes,
