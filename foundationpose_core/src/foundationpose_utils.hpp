...
struct FoundationPosePipelinePackage : public async_pipeline::IPipelinePackage 
{
  ...
  // 保存refine阶段用的推理缓存
  std::shared_ptr<infer_core::IBlobsBuffer> refiner_blobs_buffer;
  // 保存score阶段用的推理缓存
  std::shared_ptr<infer_core::IBlobsBuffer> scorer_blobs_buffer;
  // 保存用于推理的blob_buffer
  ...
  std::shared_ptr<infer_core::IBlobsBuffer> GetInferBuffer() {
    return infer_buffer;
  }
};
