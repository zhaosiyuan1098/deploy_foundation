...
std::pair<void*, DataLocation> infer_core::TrtBlobBuffer::getOuterBlobBuffer(const std::string& blob_name)
{
...
bool infer_core::TrtBlobBuffer::setBlobBuffer(const std::string& blob_name, void* data_ptr, DataLocation location)
{
    if (outer_map_blob2ptr_.find(blob_name) == outer_map_blob2ptr_.end())
    {
...
bool infer_core::TrtBlobBuffer::setBlobBuffer(const std::string& blob_name, DataLocation location)
{
    if (outer_map_blob2ptr_.find(blob_name) == outer_map_blob2ptr_.end())
    {
        LOG(ERROR) << "[TrtBlobBuffer] `SetBlobBuffer` Got invalid `blob_name`: " << blob_name;
...
bool infer_core::TrtBlobBuffer::setBlobShape(const std::string& blob_name, const std::vector<int64_t>& shape)
{
...
const std::vector<int64_t>& infer_core::TrtBlobBuffer::getBlobShape(const std::string& blob_name) const
{
    if (map_blob_name2shape_.find(blob_name) == map_blob_name2shape_.end())
    {
...
size_t infer_core::TrtBlobBuffer::size()
{
    return outer_map_blob2ptr_.size();
}

void infer_core::TrtBlobBuffer::release()
{
...
void infer_core::TrtBlobBuffer::reset()
{
...
infer_core::TrtBlobBuffer::~TrtBlobBuffer()
{
    // release device buffer
    for (void *ptr : device_blobs_buffer_)
    {
